...
public class NotifyerChannelDictionaryConverter : JsonConverter
{
    ...
    public override object ReadJson(JsonReader reader, Type objectType, object? existingValue, JsonSerializer serializer)
    {
        ...
        foreach (KeyValuePair<string, JToken?> kvp in jsonObject)
        {
            ...
            if (notifyerObject is null)
            {
                throw new JsonSerializationException("The notifyer object is null.");
            }

            var notifyer = notifyerObject.ToObject(
                               GetFullType(notifyerObject["Type"]?.ToString() ?? string.Empty),
                               serializer) as INotifyer;
            if (notifyer is null)
            {
                throw new JsonSerializationException("The notifyer object is null.");
            }
    ...
    private Type GetFullType(string typeName)
    {
        ...
        Type? type = assemblies.Select((Assembly assembly) => assembly.GetType(typeName, false, true)).FirstOrDefault((t) => t != null);
        if (type is null)
        {
